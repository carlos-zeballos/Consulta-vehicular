/**
 * MTC CITV ADAPTER CON PROXY - Versión Final
 * Usa el método que ya funciona (mtcAdapter) pero con proxy HTTP 2334
 * Formato exacto: http://USERNAME:PASSWORD@na.proxy.2captcha.com:2334
 */
...
/**
 * Construir cliente HTTP con cookie jar y proxy HTTP 2334
 */
function buildClient() {
  const jar = new CookieJar();
  
  // Configurar proxy HTTP 2334 (formato exacto de 2Captcha)
  const PROXY_HOST = process.env.MTC_PROXY_HOST || 'na.proxy.2captcha.com';
  const PROXY_PORT = '2334'; // SIEMPRE 2334 (HTTP)
  ...
  // Formato EXACTO: http://USERNAME:PASSWORD@HOST:PORT
  const proxyUrl = `http://${PROXY_USER}:${PROXY_PASS}@${PROXY_HOST}:${PROXY_PORT}`;

  console.log(`[MTC-PROXY] Configurando proxy HTTP 2334: ${PROXY_HOST}:${PROXY_PORT}`);
  console.log(`[MTC-PROXY] Usuario: ${PROXY_USER.substring(0, 30)}...`);
  ...
  // NOTA: axios-cookiejar-support no funciona con agentes personalizados
  // Por eso creamos el cliente sin wrapper y manejamos cookies manualmente
    ...
    if (setCookieHeaders.length > 0) {
      try {
        setCookieHeaders.forEach(cookieHeader => {
          jar.setCookieSync(cookieHeader, BASE_URL);
        });
